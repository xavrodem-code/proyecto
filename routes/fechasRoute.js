...
router.post("/api/fechas", async (req, res, next) => {
    ...
    if (!buscarUsuario) {
        const error = new Error("No se ha podido encontrar el usuario con el ID proporcionado");
        error.code = 404;
        return next(error);
    }
...
router.patch("/api/fechas/:id", async (req, res, next) => {
    ...
    try {
        fecha = await Fecha.findByIdAndUpdate(idFecha, req.body, {new:true, runValidators: true,})
    } catch (err) {
        res.status(404).json({
